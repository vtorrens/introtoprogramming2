function OnlineAttitudes() {
    ...
    this.preload = function() {
      var self = this;
      data = loadTable(
        './data/Online-attitudes/onlineAttitudes.csv', 'csv', 'header',
  ...
  console.log(this.waffles)
	...
	this.setup = function setup() {
		...
		values = ['Will Never Do it','Very Negative', 'Negative', 'Neutral',
			'Positive', 'Very Positive'];
	
		for (var i = 0; i < attitudes.length; i++){
			if (i< 4)
			this.waffles.push(new Waffle(150 + (i*300), 200, 200, 200, 10, 10, data, attitudes[i], values, attitudes [i] ));
			else {
					this.waffles.push(new Waffle(120 + (i-4)*220, 240, 200, 200, 10, 10, data, attitudes[i], values));
			}
		}	
	}
...
//    pop();
  ...
  function Waffle(x, y, width, height, boxes_across, boxes_down, table, columnHeading, possibleValues) {
    ...
    this.addCategories = function (){
        ...
        for(var i = 0; i < this.categories.length; i ++){
            this.categories[i].boxes = 
            round((this.categories[i].count / this.column.length) * (this.boxes_down * this.boxes_across));
        }
    }
    this.addBoxes = function(){
      ...
      for (var i = 0; i < this.boxes_down; i++) {
          ...
          for (var j = 0; j < this.boxes_across; j++){
              ...
              this.boxes[i].push (new Box (this.x + (j * boxWidth), this.y + (i * boxHeight), 
                                       boxWidth, boxHeight, this.categories[currentCategory]));
                                       currentCategoryBox++;
          }
       }  
    }
